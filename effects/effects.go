...
func Sepia(img *image.RGBA) *image.RGBA {
	...
	utils.ForEachRGBAPixel(img, func(pixel color.RGBA, x, y int) {
		r := float64(pixel.R)
		g := float64(pixel.G)
		b := float64(pixel.B)
		...
		res.SetRGBA(x, y, color.RGBA{
			R: uint8(utils.ClampF64(resR, utils.MinUint8, float64(utils.MaxUint8))),
			G: uint8(utils.ClampF64(resG, utils.MinUint8, float64(utils.MaxUint8))),
			B: uint8(utils.ClampF64(resB, utils.MinUint8, float64(utils.MaxUint8))),
			A: pixel.A,
		})
	})
	return res
...
func InvertGray(img *image.Gray) *image.Gray {
	size := img.Bounds().Size()
	inverted := image.NewGray(image.Rect(0, 0, size.X, size.Y))
	utils.ForEachGrayPixel(img, func(pixel color.Gray, x, y int) {
		inverted.SetGray(x, y, color.Gray{Y: utils.MaxUint8 - pixel.Y})
	})
	return inverted
}
...
func InvertRGBA(img *image.RGBA) *image.RGBA {
	size := img.Bounds().Size()
	inverted := image.NewRGBA(image.Rect(0, 0, size.X, size.Y))
	utils.ForEachRGBAPixel(img, func(pixel color.RGBA, x, y int) {
		inverted.SetRGBA(x, y, color.RGBA{
			R: utils.MaxUint8 - pixel.R,
			G: utils.MaxUint8 - pixel.G,
			B: utils.MaxUint8 - pixel.B,
			A: pixel.A,
		})
	})
	return inverted
