...
func Thin(img *image.Gray) (*image.Gray, error) {
	...
	utils.ForEachPixel(img, func(c color.Color, x, y int) {
		res.Set(x, y, c)
	})
	...
	for {
		...
		// (c) P2 * P4 * P8 = 0
		// (d) P2 * P6 * P8 = 0
		utils.ForEachPixel(res, func(c color.Color, x, y int) {
			// Only evaluate filled positions.
			if c == White {
				return
			}

			numFilledNeighbors, numNeighbors := countNeighbors(res, x, y)
			...
			// Condition (b)
			numZeroOnePatterns := countZeroOnePatterns(res, x, y)
			...
			if alternateSubIterations {
				// Condition (c)
				p2 := digitizedNeighbor(img, x, y, 2)
				p4 := digitizedNeighbor(img, x, y, 4)
				p6 := digitizedNeighbor(img, x, y, 6)
				if (p2 * p4 * p6) != 0 {
					return
				}

				// Condition (d)
				p8 := digitizedNeighbor(img, x, y, 8)
				if (p4 * p6 * p8) != 0 {
					return
				}
			} else {
